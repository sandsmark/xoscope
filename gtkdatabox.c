...
struct _GtkDataboxPrivate {
    ...
    /* values that correspond to INT16_MIN/MAX pixel coordinates */
    gfloat lower_limit_x;
    gfloat upper_limit_x;
    gfloat lower_limit_y;
    gfloat upper_limit_y;
...
gtk_databox_calculate_translation_factors (GtkDatabox * box) {
    ...
    if (priv->translation_factor_x > 0) {
        priv->lower_limit_x = gtk_databox_pixel_to_value_x(box, INT16_MIN);
        priv->upper_limit_x = gtk_databox_pixel_to_value_x(box, INT16_MAX);
    } else {
        priv->lower_limit_x = gtk_databox_pixel_to_value_x(box, INT16_MAX);
        priv->upper_limit_x = gtk_databox_pixel_to_value_x(box, INT16_MIN);
    }

    if (priv->translation_factor_y > 0) {
        priv->lower_limit_y = gtk_databox_pixel_to_value_y(box, INT16_MIN);
        priv->upper_limit_y = gtk_databox_pixel_to_value_y(box, INT16_MAX);
    } else {
        priv->lower_limit_y = gtk_databox_pixel_to_value_y(box, INT16_MAX);
        priv->upper_limit_y = gtk_databox_pixel_to_value_y(box, INT16_MIN);
    }
}
...
gtk_databox_expose (GtkWidget * widget, GdkEventExpose * event) {
    ...
    cr = cairo_create(priv->backing_surface);
    gdk_cairo_set_source_color (cr, &gtk_widget_get_style(widget)->bg[0]);
    cairo_paint(cr);
    cairo_destroy(cr);
...
gtk_databox_get_offset_y (GtkDatabox* box) {
    ...
    if (priv->total_bottom == priv->total_top)
        return 0;
...
gtk_databox_values_to_pixels (GtkDatabox * box, guint len,
    ...
    for (i = 0; i < len; ++i, ++values_x, ++values_y, ++pixels) {
        pixels->x = gtk_databox_value_to_pixel_x(box, *values_x);
        pixels->y = gtk_databox_value_to_pixel_y(box, *values_y);
    }
}
...
gtk_databox_value_to_pixel_x (GtkDatabox * box, gfloat value) {
    ...
    if (priv->translation_factor_x > 0) {
        if (value < priv->lower_limit_x) {
            return INT16_MIN;
        }
        if (value > priv->upper_limit_x) {
            return INT16_MAX;
        }
    } else {
        if (value < priv->lower_limit_x) {
...
gtk_databox_value_to_pixel_y (GtkDatabox * box, gfloat value) {
    ...
    if (priv->translation_factor_y > 0) {
        if (value < priv->lower_limit_y) {
            return INT16_MIN;
        }
        if (value > priv->upper_limit_y) {
            return INT16_MAX;
        }
    } else {
        if (value < priv->lower_limit_y) {
