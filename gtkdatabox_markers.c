				   ...
				   GtkDatabox* box)
{
   ...
   if (cr)
   {
      if (GTK_DATABOX_MARKERS_GET_PRIVATE (markers)->type == GTK_DATABOX_MARKERS_DASHED_LINE)
                            ...
                            gint distance, gboolean boxed, GdkColor *color, int size)
{
    ...
    if (boxed)
    {
        ...
        cairo_set_source_rgb (cr, color->red/65535.,color->green/65535.,color->blue/65535.);
        cairo_rectangle (cr, coord.x + hdist_box - 0.5, coord.y + vdist_box - 0.5f, width + 3.5, height + 3.5);
        cairo_stroke (cr);
        cairo_restore(cr);
    }
                               ...
                               GtkDatabox* box)
{
   ...
   for (i = 0; i < len; ++i)
   {
	 ...
	 gtk_databox_label_write_at (cr,
				     ...
				     priv->markers_info[i].boxed, color, 1);
      }
   }
