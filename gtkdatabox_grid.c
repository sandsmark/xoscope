			    ...
			    GtkDatabox* box)
{
   ...
   gfloat visible_left, visible_right, visible_top, visible_bottom;
   ...
   gtk_databox_get_visible_limits (box, &visible_left, &visible_right, &visible_top, &visible_bottom);
   ...
   /* For dotted or dashed grids, we now compute spacing between the
    * dots or dashes.
    *
    ...
    *
    * The value-to-pixel functions return an int16, so we need to be
    * careful on large databoxes not to call them for values far
    * outside the visible range.  This is why we figure the first and
    * last visible lines and base our computations on them.
    */

   target_spacing = 4.0 + cairo_get_line_width(cr);

   first_line = floor((visible_left - offset_x)/factor_x);
   last_line = ceil((visible_right - offset_x)/factor_x);

   pixel_right = gtk_databox_value_to_pixel_x (box, offset_x + last_line*factor_x);
   pixel_left = gtk_databox_value_to_pixel_x (box, offset_x + first_line*factor_x);

   grid_spacing = (pixel_right - pixel_left)/(last_line - first_line);
   ...
   for (i = 0; i < priv->hlines; i++) {
     ...
     if (((y >= visible_bottom) && (y <= visible_top))
   ...
   first_line = floor((visible_bottom - offset_y)/factor_y);
   last_line = ceil((visible_top - offset_y)/factor_y);

   pixel_bottom = gtk_databox_value_to_pixel_y (box, offset_y + first_line*factor_y);
   pixel_top = gtk_databox_value_to_pixel_y (box, offset_y + last_line*factor_y);

   grid_spacing = (pixel_bottom - pixel_top)/(last_line - first_line);
