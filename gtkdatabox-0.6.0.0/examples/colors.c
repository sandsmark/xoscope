...
menu_color_change_cb (col_sel *sel)
{
   ...
   selector = GTK_COLOR_SELECTION_DIALOG (gtk_color_selection_dialog_new (title));
   ...
   g_signal_connect_object (G_OBJECT
			    (selector->cancel_button), "clicked",
			    G_CALLBACK (gtk_widget_destroy),
			    G_OBJECT (selector), G_CONNECT_SWAPPED);
   g_signal_connect (G_OBJECT
		     (selector->ok_button),
		     "clicked", G_CALLBACK (get_color_cb), (gpointer) sel);
   gtk_widget_show (GTK_WIDGET (selector));

   return;
}

static void create_menu_entry (GtkWidget *menu, int index, 
